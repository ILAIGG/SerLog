...
def actualizar_resultados():
    if 'datos_logistica' in globals():
        ...
        costo_combustible = datos_logistica["costo_combustible"] * 10  # Supongamos 10 litros
        ...
        lbl_fecha.config(text=f"Fecha: {datetime.now().strftime('%d/%m/%Y')}")
        lbl_hora.config(text=f"Hora: {datos_logistica['horario']}")
...
# Frame de Ingresar Datos (modificado)
