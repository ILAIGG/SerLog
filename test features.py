...
# desplegable de camiones
# --- 1. Sección: Tipo de Camión ---
frame_camion = tk.LabelFrame(panel_izquierdo, text="Tipo de camión", bg="#f0f0f0")
frame_camion.pack(padx=10, pady=(10, 5), fill=tk.X)

opciones_camion = ["Camión chico", "Camión mediano", "Camión grande"]
tipo_camion_var = tk.StringVar()
tipo_camion_var.set(opciones_camion[0])

menu_camion = tk.OptionMenu(frame_camion, tipo_camion_var, *opciones_camion)
menu_camion.pack(padx=5, pady=5, fill=tk.X)


# --- 2. Sección: Fecha de Partida ---
frame_fecha = tk.LabelFrame(panel_izquierdo, text="Fecha de partida", bg="#f0f0f0")
frame_fecha.pack(padx=10, pady=(0, 5), fill=tk.X)

entry_fecha = tk.Entry(frame_fecha, width=20)
entry_fecha.pack(padx=5, pady=5, fill=tk.X)


# --- 3. Sección: Horario de Salida ---
frame_hora = tk.LabelFrame(panel_izquierdo, text="Horario de salida", bg="#f0f0f0")
frame_hora.pack(padx=10, pady=(0, 5), fill=tk.X)

entry_hora = tk.Entry(frame_hora, width=20)
entry_hora.pack(padx=5, pady=5, fill=tk.X)


# --- 4. Sección: Direcciones y Ruta ---
frame_direccion = tk.LabelFrame(panel_izquierdo, text="Direcciones", bg="#f0f0f0")
frame_direccion.pack(padx=10, pady=(0, 5), fill=tk.BOTH, expand=True)

label = tk.Label(frame_direccion, text="Dirección:", bg="#f0f0f0", anchor="w")
label.pack(padx=5, pady=(5, 0), anchor="w")

entry_direccion = tk.Entry(frame_direccion, width=35)
entry_direccion.pack(padx=5, pady=5)

btn_buscar = tk.Button(frame_direccion, text="Agregar marcador", command=buscar_direccion)
btn_buscar.pack(padx=5, pady=5)

btn_ruta = tk.Button(frame_direccion, text="Calcular Ruta Óptima", bg="lightblue", command=calcular_ruta_optima)
btn_ruta.pack(padx=5, pady=5)

label_lista = tk.Label(frame_direccion, text="Direcciones agregadas:", bg="#f0f0f0", anchor="w")
label_lista.pack(padx=5, pady=(5, 0), anchor="w")

# Lista con scroll
frame_lista = tk.Frame(frame_direccion)
frame_lista.pack(padx=5, pady=5, fill=tk.BOTH, expand=True)
